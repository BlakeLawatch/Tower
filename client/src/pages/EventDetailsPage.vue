    ...
    <div v-if="activeEvent" class="container-fluid">
        <section class="row">
            <div class="col-12 col-md-4 my-4">
                ...
                <img v-if="activeEvent" class="img-fluid img-cancelled rounded" :src="activeEvent.coverImg"
                    alt="Your event">
                <img v-if="activeEvent.capacity == 0" class="img-fluid img-cancelled rounded" :src="activeEvent.coverImg"
                    alt="Your event">
                ...
                <div class="d-flex justify-content-around my-2">
                    <button v-if="!activeEvent.isCanceled && activeEvent.capacity >= 1" class="btn btn-warning"
                        @click="createTicket()">Buy a Ticket</button>
                    <button v-if="activeEvent.creatorId == account.id && activeEvent.isCanceled == false"
                        @click="cancelEvent()" class="btn btn-danger">Cancel Event</button>
                </div>
            </div>
            ...
            <div v-for="ticket in tickets" :key="ticket" class="col-3 ms-3 p-3 card-bg rounded text-light">
                <img class="rounded-circle profile-img m-1" :title="ticket.profile.name" :src="ticket.profile.picture"
        ...
        <section>
            ...
            <div v-for="comment in comments" :key="comment" class="col-12 text-light m-2 d-flex comment-card rounded p-3">
                <div class="col-5">
                    <img class="profile-img rounded-circle my-1 img-fluid" title="comment.creator.name"
                ...
                <div class="col-7 shadow rounded p-2">
                    ...
                    <button v-if="comment.creatorId == account.id" @click="destroyComment(comment.id)"
                        class=" text-end btn btn-danger mdi mdi-delete-empty"></button>
                </div>
            </div>
